...
def generate_anomaly_event():
    anomaly_type = random.choice(['ddos', 'portscan', 'data_leak', 'bgp'])
    ...
    if anomaly_type == 'ddos':
        ...
        event['dst_port'] = 179
        event['protocol'] = 'BGP'
        event['src_ip'] = f"172.{random.randint(16,31)}.{random.randint(0,255)}.{random.randint(1,254)}"
        event['bytes'] = random.randint(1000, 8000)
        event['packets'] = random.randint(5, 30)

    event['is_anomaly'] = 1
    return event

print("Продюсер v2 запущен. Отправляем события в Kafka...")
count = 0

while True:
    if random.random() < 0.05:
        event = generate_anomaly_event()
        anomaly_type = 'BGP' if event['dst_port'] == 179 else 'АНОМАЛИЯ'
        print(f"[{anomaly_type}] {event['timestamp']} | {event['src_ip']} -> {event['dst_ip']} | bytes:{event['bytes']}")
    else:
        event = generate_normal_event()
